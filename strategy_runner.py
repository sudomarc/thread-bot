...
STRATEGY_MIX = {
    "builder_experience": 8, "humor": 4, "opinion_observation": 4,
    "question": 2, "news_explainer": 1, "gaming": 1,
}
...
ARTICLE_RELEVANCE_KEYWORDS = {
    "builder_experience": {"ai","artificial intelligence","developer","coding","code","software","automation","workflow","model","open source","api","cloud","gpu","chip","semiconductor","startup","robotics","tool","platform","computer","hardware","infrastructure"},
    "opinion_observation": {"ai","artificial intelligence","developer","coding","software","automation","model","openai","anthropic","nvidia","google","microsoft","apple","cloud","startup","open source"},
...
def _article_relevance(article, strategy_kind):
    ...
    return sum(1 for keyword in ARTICLE_RELEVANCE_KEYWORDS.get(strategy_kind, set()) if keyword in text)
...
def _pick_article(articles, category, strategy_kind=None):
    ...
    if strategy_kind:
        ranked = sorted(enumerate(candidates), key=lambda pair: (_article_relevance(pair[1], strategy_kind), -pair[0]), reverse=True)
        if _article_relevance(ranked[0][1], strategy_kind) > 0:
            return ranked[0][1]
    return candidates[0]
...
def _editorial_brief_from_slot(recipe, relatable_topic):
    ...
    parts = [
        f"Format: {kind}.", f"Hook direction: {hook}.", f"Editorial instruction: {instruction}.",
...
def _engagement_pattern(kind, hook):
    return ENGAGEMENT_PATTERN_BY_HOOK.get(hook, {
        "builder_experience": "CONSTRAINT_WORKAROUND", "humor": "RECOGNITION_HUMOR",
        "opinion_observation": "POSITION", "question": "OPEN_QUESTION",
        "news_explainer": "DISCOVERY", "gaming": "SCENARIO_CHOICE",
    }.get(kind, kind.upper()))

def record_performance(state, data):
    row = performance_row(data)
    row["normalized"] = normalized_metrics(row)
    state.setdefault("performance_history", []).append(row)
    state["performance_history"] = state["performance_history"][-200:]
    return row
...
def _provider_stage(prompt):
    ...
    if text.startswith("fact-check"):
        return "fact_check"
    if text.startswith("generate at least 8 materially"):
        return "angles"
    if text.startswith("write one natural"):
        return "draft"
    return "unknown"
...
def _instrument_provider(provider, attempt):
    def call(prompt):
        ...
        print(f"PIPELINE event=provider_request stage={stage} provider=openrouter model=openrouter/free attempt={attempt}")
        try:
            content = provider(prompt)
        except Exception as error:
            print(f"PIPELINE event=provider_response stage={stage} outcome=error error_kind={_provider_error_kind(error)}")
            raise
        print(f"PIPELINE event=provider_response stage={stage} outcome=ok content_chars={len(content) if isinstance(content, str) else -1}")
        return content
...
def _diagnose_result(result):
    ...
    if str(diagnosed.get("decision", "UNKNOWN")).upper() != "REJECT":
        return diagnosed
    ...
    if angles and not diagnosed.get("top_pick"):
        ...
        diagnosed["rejection_reason"] = (
            ...
            f"best_idea_decision={best.get('idea_decision', 'REJECT')} required_score>=80 and hard dimensions {hard} must each be >=7/10."
        )
    ...
    if stress and not bool(stress.get("all_pass", False)):
        ...
        diagnosed["rejection_reason"] = "Stress test failed: " + ", ".join(failed or ["unknown_check"]) + "."
        diagnosed["recoverable"] = True
        return diagnosed
...
def _log_pipeline_result(result):
    ...
    print(f"PIPELINE event=parsed_result outcome=ok decision={result.get('decision', 'UNKNOWN')} keys={','.join(sorted(str(key) for key in result.keys()))}")
    if result.get("fact_gate") is not None:
        print(f"PIPELINE event=validation_result stage=fact_check outcome={'reject' if result.get('stage') == 'fact_check' else 'pass'} fact_confidence={result.get('fact_confidence', 'unknown')}")
    angles = result.get("angles") or []
    if angles:
        eligible = sum(1 for angle in angles if angle.get("idea_decision") in {"EXCEPTIONAL", "STRONG", "PROMISING"})
        print(f"PIPELINE event=quality_scoring stage=idea_selection angles={len(angles)} eligible={eligible} best_idea_score={max(float(a.get('idea_score', 0)) for a in angles):.1f}")
    if "quality_score" in result or "final_score" in result:
        print(f"PIPELINE event=quality_scoring stage=final quality_score={result.get('quality_score', 'unknown')} final_score={result.get('final_score', 'unknown')}")
    return result
...
def _write_pipeline_report(result):
    ...
    with open(PIPELINE_REPORT_PATH, "w", encoding="utf-8") as handle:
        handle.write(
            ...
            f"POST TYPE: {result.get('post_type', 'LEGACY')}\n"
...
def _retry_brief(editorial_brief, error, post_type=None):
    ...
    if "angles are repetitive" in message or "expected at least 8 angles" in message:
        return f"{editorial_brief} ANGLE DIVERSITY RETRY: regenerate from the same source facts. Return at least 10 angles using at least 8 different allowed angle types and materially different core_claims. Vary mechanism, stakeholder, consequence, incentive, behavior, trade-off, time horizon, or question. Keep facts grounded in the supplied material.".strip()
...
def _evaluate_with_diagnostics(topic, article, editorial_brief, provider, attempt, post_type=None):
    return _log_pipeline_result(evaluate_topic(
        topic, [article], _instrument_provider(provider, attempt),
        editorial_brief=editorial_brief, post_type=post_type
    ))

def _run_pipeline(topic, article, editorial_brief="", post_type=None):
    try:
        result = _evaluate_with_diagnostics(topic, article, editorial_brief, bot.openrouter_chat_json, 1, post_type)
        ...
        try:
            ...
            raise PipelineError(
                f"Pipeline retry failed after {type(first_error).__name__}: {first_error}; retry failed: {type(retry_error).__name__}: {retry_error}"
            ) from retry_error
        ...
        retry_brief = _retry_brief(editorial_brief, first_error, post_type)
        try:
            return _evaluate_with_diagnostics(topic, article, retry_brief, _retry_openrouter_chat, 2, post_type)
        except Exception as retry_error:
            raise PipelineError(f"Pipeline provider output failed: {type(first_error).__name__}: {first_error}; retry failed: {type(retry_error).__name__}: {retry_error}") from retry_error
    result = _diagnose_result(result)
    ...
    try:
        return _diagnose_result(_evaluate_with_diagnostics(topic, article, retry_brief, bot.openrouter_chat_json, 2, post_type))
    except Exception as retry_error:
        raise PipelineError(f"Pipeline rejection retry failed: {type(retry_error).__name__}: {retry_error}") from retry_error
...
def generate_strategy_threads(articles, state):
    ...
    if result.get("decision") == "REWRITE":
        result = _run_pipeline(
            topic, article,
            f"{editorial_brief} Rewrite pass: preserve the strongest defensible claim, increase specificity and tension, and remove generic wording.",
            post_type=post_type,
        )
    result = _diagnose_result(result)
    if result.get("decision") != "PUBLISH":
        _write_pipeline_report(result)
        raise RuntimeError(f"Content pipeline decision: {result.get('decision', 'UNKNOWN')} stage={result.get('stage', 'unknown')} reason={result.get('rejection_reason', 'unspecified')}")
    _write_pipeline_report(result)
    ...
    state["strategy_last_engagement_pattern"] = engagement_pattern
    ...
    state["last_pipeline_scores"] = {
        "idea_score": post["idea_score"], "quality_score": post["quality_score"],
        "final_score": post["final_score"], "fact_confidence": post["fact_confidence"],
    }
    ...
    state.setdefault("recent_engagement_patterns", []).append(engagement_pattern)
    state["recent_engagement_patterns"] = state["recent_engagement_patterns"][-20:]
    print(f"Strategy slot {cursor + 1}: {kind} / {post_type} / {preferred_category or 'relatable'} pattern={engagement_pattern} idea={post['idea_score']:.1f} quality={post['quality_score']:.1f} final={post['final_score']:.1f} decision={post['decision']}")
