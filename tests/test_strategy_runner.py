...
class StrategyRunnerTests(__import__('unittest').TestCase):
    ...
    def test_pick_article_prefers_requested_category(self):
        articles = [{"category": "technology", "title": "tech", "description": "", "published": "", "url": ""}, {"category": "gaming", "title": "game", "description": "", "published": "", "url": ""}]
    ...
    def test_record_performance_normalizes_metrics(self):
        state = {}
        row = strategy_runner.record_performance(state, {"post_id": "p1", "date": "2026-09-11", "topic": "AI", "angle": "economic", "idea_score": 90, "quality_score": 85, "final_score": 88, "views": 1000, "likes": 50, "replies": 20, "reposts": 10, "quotes": 5, "follows": 10})
    ...
    def test_run_pipeline_retries_semantic_fact_validation_failure(self):
        first_error = content_engine.PipelineError("Unknown factual status: ''", stage="facts", code="UNKNOWN_STATUS")
    ...
    def test_angle_retry_brief_demands_material_diversity(self):
        error = content_engine.PipelineError("Angles are repetitive; need at least 8 materially distinct angles", stage="angles", code="INSUFFICIENT_DIVERSITY")
    ...
    def test_run_pipeline_does_not_mask_original_validation_failure_when_retry_hits_provider_error(self):
        first_error = content_engine.PipelineError("Angles are repetitive; need at least 8 materially distinct angles", stage="angles", code="INSUFFICIENT_DIVERSITY")
    ...
    def test_generate_strategy_threads_runs_fact_angle_and_draft_stages(self):
        articles = [{"category": "gaming", "title": "A current gaming story", "description": "A useful summary.", "published": "2026-09-11T10:00:00Z", "url": "https://example.com/story"}]
        ...
        for index in range(8):
            ...
            angles.append({"angle": f"angle_{index}", "core_claim": f"Distinct claim {index}", "why_it_matters": "Specific consequence", "target_reaction": "Reasonable disagreement", "supporting_facts": ["A supplied fact"], "potential_counterargument": "A reasonable counterargument", "scores": scores})
        responses = [
            ...
            json.dumps({"draft": "A specific take on why this gaming story changes the economics of the next wave of games.", "quality": {key: 9 for key in content_engine.QUALITY_WEIGHTS}, "stress": {"scroll_answer": "The opening makes a concrete claim.", "reply_example": "A reader could disagree about the business impact.", "counterargument": "Better technology does not guarantee adoption.", "generic": False, "quotable_line": "The interesting shift is who controls the constraint."}, "claims": [{"claim": "The story happened", "status": "VERIFIED", "evidence": "A useful summary.", "confidence": 9, "central": True}]})
        ]
